...
@Component({
  selector: "app-lab-add-appointments",
  templateUrl: "./lab-add-appointments.component.html",
  styleUrls: ["./lab-add-appointments.component.css"],
})
export class LabAddAppointmentsComponent implements OnInit {
  sectorList: Sector[] = [];
  ...
  searchPatient(patientMobileNumber: string) {
    this._appService.searchPatientForLab(patientMobileNumber).subscribe(
      (response: any) => {
  ...
  getDepedents(primaryPatientId: string) {
    this._appService.getPatientDependentsForLab(primaryPatientId).subscribe(
      (response: any) => {
        if (response.status == APIResponse.Success) {
  ...
  getServiceSectors() {
    this._appService.getServiceSectorsForLab().subscribe(
      (response: any) => {
        if (response.status == APIResponse.Success) {
  ...
  getServiceListForSector(sectorId) {
    this._appService.getServiceListForLab(sectorId).subscribe(
      (response: any) => {
        if (response.status == APIResponse.Success) {
          if (sectorId == 1) {
  ...
  confirmAppointment() {
    ...
    this._appService.postNewAppointmentForLab(confirmAppointmentParams).subscribe(
      (response: any) => {
        if (response.status == APIResponse.Success) {
  ...
  ViewDependentList(e) {
    this._appService.getPatientDependentsForLab(e).subscribe(
      (response: any) => {
        debugger
        if (response.status == APIResponse.Success) {
