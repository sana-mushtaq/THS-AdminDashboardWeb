...
export class PatientlistComponent implements OnInit {
    ...
    this._appService.getPatientsList().subscribe(
      (response: any) => {
        if (response.status == APIResponse.Success) {
          this.actualpatientList = Patient.getPatientsList(response);          
          this.patientList = this.actualpatientList.slice(0, 250);
          let params ={
            "data": this.actualpatientList
          }
  ...
  drawPage() {
    ...
    var filterIndexes = [0, 1, 2, 3, 4, 5, 8];
    ...
    var table = $("#example").DataTable({
      ...
      columns: [
        ...
        { data: {firstName : "firstName", lastName : "lastName", patientId:'patientId'},
          render:function(data,type){
            return '<div class="cell patientRow" datavalue="'+data.patientId+'"><div class="d-flex-center" datavalue="'+data.patientId+'"><span class="avtar3" datavalue="'+data.patientId+'"><img src="assets/images/user.svg" class="image-cover" datavalue="'+data.patientId+'"/></span> <span datavalue="'+data.patientId+'" class="grid_offlinestatus"></span><span datavalue="'+data.patientId+'" title="Lama Saleh">'+data.firstName+' '+data.lastName+'</span></div></div>';
          }
        },
        { data: 'email'},
        { data: 'mobileNumber'},
        ...
        { data: 'patientId',
          render:function(data,type){
            return '<div class="cell patientRow" datavalue="'+data+'"><i class="icon-pencil ico-pointer" id="ico-pointer" datavalue="'+data+'"></i></div>';
          }
        },
      ...
      initComplete: () => {
        $("#example").show();
        $("#example").on('click','.patientRow',(e) =>{
          // console.log(e.target.attributes.datavalue.value);
          this.viewPatientDetails(e.target.attributes.datavalue.value)
        });
