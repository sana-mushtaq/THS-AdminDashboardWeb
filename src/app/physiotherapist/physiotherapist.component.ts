...
export class PhysiotherapistComponent implements OnInit {
  ...
  constructor(
    ...
    this.getPractiseUserList(PractiseUserRoles.Physiotherapist);
  }
  ...
  careProviderStaffClicked(careProviderStaff) {
    ...
    this.router.navigate(['../view-physiotherapist']);
  }
  ...
  drawPage() {
    ...
    $('#physiotherapistTable thead tr')
      .clone(true)
      .addClass('filters')
      .appendTo('#physiotherapistTable thead');


    var filterIndexes = [0, 1, 2, 3];

    var table = $('#physiotherapistTable').DataTable({
      orderCellsTop: true,
      fixedHeader: true,
